...
def tune_hyperparams(X_train, y_train, X_val, y_val, model, params, num_threads: int = 1):
    ...
    clf = GridSearchCV(model, params, n_jobs=6, verbose=1, cv=PredefinedSplit(test_fold=test_fold), refit=False, scoring='roc_auc')
    clf.fit(X, y)
...
def main(args):
    ...
    for model_name in ['gbm', 'logistic']:
        ...
        if model_name == 'gbm':
            model = tune_hyperparams(X_train, y_train, X_valid, y_valid, ltb.LGBMClassifier(), XGB_PARAMS, num_threads = args.num_threads)
        elif model_name == 'logistic':
            model = tune_hyperparams(X_train, y_train, X_valid, y_valid, sklearn.linear_model.LogisticRegression(), LR_PARAMS, num_threads = args.num_threads)

        with open(os.path.join(args.path_to_output_dir, f'{model_name}_model.pkl'), 'wb') as f:
            pickle.dump(model, f)
        
        proba = model.predict_proba(features)[:, 1]

        with open(os.path.join(args.path_to_output_dir, f'{model_name}_predictions.pkl'), 'wb') as f:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train baselines logistic regression and lightgbm models")
